...
import { DeepPartial, FindManyOptions, FindOneOptions, FindOptionsWhere, InsertResult } from 'typeorm';
...
export abstract class BaseService<T extends BaseEntity> implements IBaseService<T> {
  ...
  async upsert(dto: QueryDeepPartialEntity<T> | QueryDeepPartialEntity<T>[], conflictPathsOrOptions: string[] | UpsertOptions<T>): Promise<T> {
    try {
      const insertResult =  await this.repository.upsert(dto, conflictPathsOrOptions)
  ...
  async findAndCount(options?: FindManyOptions<T>): Promise<{
    data: T[];
    total: number;
  }> {
    try {
  ...
  async create(dto: DeepPartial<T>): Promise<T> {
    try {
      ...
      this.logger.log(`Successfully created entity with id: ${created.id}`);
      return created;
    } catch (error) {
  ...
  async update(id: string, dto: QueryDeepPartialEntity<T>): Promise<T> {
    try {
      ...
      const updated = await this.repository.update({ id: +id } as unknown as FindOptionsWhere<T>, updateData);
  ...
  async remove(id: string): Promise<void> {
    try {
      ...
      this.logger.log(`Successfully soft deleted entity with id: ${id}`);
    } catch (error) {
  ...
  async restore(id: string): Promise<void> {
    try {
      ...
      this.logger.log(`Successfully restored entity with id: ${id}`);
    } catch (error) {
  ...
  protected handleError(operation: string, error: any): never {
    ...
    if (error.code === '23505') {
      ...
      throw new BadRequestException('Entity with these details already exists');
    }

    throw new InternalServerErrorException(`Error performing ${operation} operation. Please try again later.`);
  }
